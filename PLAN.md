...
- **Implemented and published:** the recommended customized dashboard contains the full September content: 21 charts, six datasets, inclusive From month / Through month controls and separate Month / Quarter / Year grouping. The official Superset 6.1.0 option contains the same dashboard content with native month selectors. The public overview at source `d98abb173976997037d7e61e4be03f23670aaeaa` links both dashboards, their matching logins, the comparison and the evidence. No reporting records were restored or reseeded.
- **Validated publicly:** the customized dashboard passes 15 applicable public workflows and the official option passes six. The evidence includes every one of the 11 date axes at 1024, 1280 and 1600 pixels, all nine contents links, the 21-panel opening, missing versus zero, partial-quarter boundaries, both selection orders and same-page clear/reselect. Seven dashboard recordings have 58 reviewed checkpoints. Six public overview checks pass, including the password copy controls and exact login destinations.
- **Definition and editor validation:** alternating imports preserve the established dashboards. In an isolated official 6.1.0 editor rehearsal, separate dataset, chart and dashboard imports restored all 30 definition files after interface edits without changing object counts or unrelated definitions. Reporting records remained separate throughout.
- **Recommendation:** retain the customized dashboard as the client candidate because it meets the agreed wording and interaction criteria. The official option proves that inclusive native month selection can work without CSiM application code, but currently retains year-first labels, saved fixed-month defaults, horizontal controls and no warning for a reversed month range. These are explicit comparison gaps.
- **CI correction:** exact-revision run `34676655114` passed both official comparison jobs and the preview job. Its corrected job passed source generation, imports, overview, redirects, supplied data, known records, the independent fixture and the complete recommended September month workflow. GitHub then stopped the final preserved-dashboard regression at the former 90-minute job limit while its assertions were still passing. The job allowance is now 120 minutes; no acceptance case or assertion timeout was removed.
- **Remaining:** replacement exact-head CI after the job-limit correction, GitHub review, client confirmation of the editor handover, the unresolved reporting-rule inputs and Beth's separate acceptance. An intermittent rapid-keyboard interaction remains an observation; the tested ordinary pointer workflow recovers on the same page.
...
Keep ongoing work and generated evidence in this permanent checkout. Preserve accepted evidence in durable release storage or GitHub artifacts with its source revision and rollback record; temporary directories must never be the sole copy of work needed for delivery.
...
| Monthly labels are omitted or squeezed at chart edges | 3 | Exact Month/Quarter/Year label lists and collision bounds on all 11 axes at 1024, 1280 and 1600 pixels | All 99 cases pass on both custom bases locally; public all-eleven hover checks pass. Public 99-case matrix and screenshot review complete; antibiotic title spacing passes. Some All/Inv legend controls still clip |
