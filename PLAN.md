...
- **CI correction:** the development-preview job passed same-page recovery, all 20 charts and all 11 date axes, then timed out waiting for the entire Superset page to become network-idle before checking saved defaults. Dashboard readiness now uses the visible controls and complete chart inventory; each workflow still waits for its own responses and settled chart pixels. The exact failed public preview case and the recommended dashboard's recovery/default cases pass with this correction.
- **Remaining:** replacement exact-head CI, GitHub review, client confirmation of the editor handover, the unresolved reporting-rule inputs and Beth's separate acceptance. An intermittent rapid-keyboard interaction remains an observation; the tested ordinary pointer workflow recovers on the same page.
