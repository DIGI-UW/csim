...
test('Copy icons copy the password for the matching dashboard',async({page,context},info)=>{
  ...
  for(const [name,file] of [['main','access.json'],['preview','preview-access.json']]){
...
test('Readers can open the September dashboard and find each matching login',async({page})=>{
  ...
  expect(recommended.searchParams.get('next')).toBe('/superset/dashboard/csim-individual-reconciled/');
  ...
  expect(destination.searchParams.get('next')).toBe('/superset/dashboard/csim-individual-reconciled/');
});
...
test('Version comparison does not pass custom installations off as standard Superset',async({page})=>{
  ...
  for(const row of [rows.nth(0),rows.nth(1)]){
    await expect(row).toContainText('Separate demo not available yet.');
    await expect(row.locator('a')).toHaveCount(0);
  }
  await expect(rows.nth(2)).toContainText('CSiM custom');
  await expect(rows.nth(2).locator('a')).toHaveCount(2);
  await expect(page.locator('#solution-labels')).toContainText('some monthly labels disappear');
  await expect(page.locator('#solution-filters')).toContainText('expected hospital-only results returned without reloading');
  await expect(page.locator('#solution-calculation')).toContainText('Accurate filtering does not depend on using them');
...
test('Issue navigation and older section links have valid destinations',async({page})=>{
  ...
  await expect(page.locator('#solution-labels')).toContainText('Extra spacing code has not shown a clear advantage');
