...
import {profile,chartCount,reconciled} from '../acceptance.config.mjs';
...
test(`01 All ${chartCount} charts load or report legitimate empty results`,async({page},info)=>{
  ...
  for(let i=0;i<chartCount;i++){
    ...
    if([0,4,8,12,16,19].includes(i))await scene(page,info,`section-${i}`,title,reconciled?'Beth’s September layout contains 21 charts, including the latest-data card.':'The full April dashboard retains all 20 charts with their original measures and layout.',i===0?'Full dashboard':undefined);
  }
