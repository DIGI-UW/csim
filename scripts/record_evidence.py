...
p.add_argument('target',choices=['main','examples','preview'])
p.add_argument('--base-url')
p.add_argument('--output',type=Path)
a=p.parse_args()
profile='preview' if a.target=='preview' else 'corrected'
ids=['01','05'] if a.target=='main' else ['05'] if a.target=='preview' else ['02','03','04','06']
...
data=ROOT/'data'/('edge-cases.sql' if a.target=='examples' else 'v1_schema_dump.sql')
