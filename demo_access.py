"""Provision a shareable account limited to the CSiM demo datasets."""
import json
import secrets
...
with app.app_context():
    ...
    databases=db.session.query(Database).filter(Database.database_name.in_(['CSiM demo PostgreSQL','CSiM edge-case PostgreSQL'])).all()
    for dataset in db.session.query(SqlaTable).filter(SqlaTable.database_id.in_([database.id for database in databases])):
