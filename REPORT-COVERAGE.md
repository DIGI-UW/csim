...
| Edge labels are squeezed or omitted | The CSiM formatter selects ticks at real reporting periods, keeps both endpoints, centers edge labels with padding and reduces intermediate labels when width requires it. The six stacked-bar legends sit above the plots to preserve horizontal space. Browser checks measure the painted text and capture all eleven axes at 1024, 1280 and 1600 pixels. | Intermediate labels can be omitted for readability; observations and hover details remain available. |
| Empty panels in the default selection | The corrected demo starts with hospital 53, including both hospital selectors; all twenty established panels and all twenty-one September-version panels are checked. The known-record versions start with hospital 91. | Cohort does not have an individual-hospital latest comparison or hospital total. Saved filter links retain their saved selection. Genuine missing periods remain gaps. |
