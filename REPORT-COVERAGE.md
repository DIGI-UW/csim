...
The recommended dashboard uses official Superset 6.1.0 with native month controls, year-first labels and a reporting-period summary. It retains all 21 September reporting charts; the summary adds a 22nd panel. See [native controls](NATIVE_CONTROLS.md) for behavior, maintenance and the specific differences from the custom alternatives.
...
| Labels do not follow Month/Quarter/Year | Official Superset uses saved dataset labels: 2025-01, 2025 Q1 and 2025, with filtering before grouping. The custom alternative displays Jan 2025, Q1 2025 and 2025. Both cover eleven date axes, including the five priority trends. | The official option has different month and quarter wording. The custom formatter is an alternative for the requested wording; native alternatives have not been exhaustively ruled out. |
...
| Edge labels are squeezed or omitted | The CSiM formatter selects ticks at real reporting periods, keeps both endpoints, centers edge labels with padding and retains every monthly label. The six stacked-bar legends sit above the plots to preserve horizontal space. Browser checks measure the painted text and capture all eleven axes at 1024, 1280 and 1600 pixels. | The official option retains all periods using year-first wording and rotation. Its native label completeness and geometry are asserted separately from the original wording comparison. |
| Empty panels in the default selection | The recommended September dashboard opens in Cohort with the numeric Your hospital selector unset. Cohort charts render results; the official dashboard places permanent selection instructions beside native empty states. Hospital query guards prevent an all-hospital total when no hospital is selected. The custom option adds inline prompts. The known-record versions use explicit hospital selections. | A hospital-only result requires a hospital. A population/window with no observations remains legitimately empty, and missing periods remain gaps. |
| Time Period and Time Unit interact inconsistently | Native horizontal controls pass both selection orders, clear/reselect on the same page and rolling opening defaults. The reporting summary explains reversed dates after Apply and partial quarters/years. SQL filters observations before grouping. | These tests do not establish the cause of every historical intermittent permalink or authoring failure. |
...
## Date handling in the custom alternative
